...
extends Check {
    ...
    public LineOfSite(AntiCheat antiCheat) {
        super(antiCheat, CheckType.KILLAURA, "VisibleOld", "KillAura (VisibleOld)", 1, 50, 2, 0);
    }
    ...
    public void Damage(EntityDamageByEntityEvent entityDamageByEntityEvent) {
        ...
        PlayerStats playerStats = this.getCore().getPlayerStats(player2);
        if (!this.isEnabled()) {
            return;
        }
        if (player.isInsideVehicle() || player2.isInsideVehicle()) {
            return;
        }
        if (player2.getLocation().getY() < 0.0 || player2.getLocation().getY() > (double)player2.getWorld().getMaxHeight()) {
            return;
        }
        if (playerStats.getLastMountDiff() < 500) {
            return;
        }
        Location location = PlayerUtil.getEyeLocation(player);
        Location location2 = PlayerUtil.getEyeLocation(player2);
        ...
        if ((d = location.getZ() - location2.getZ()) < -1.0 || d > 1.0) {
        ...
        if (n6 > n7) {
            this.getCore().addViolation(player, this, new Violation(this, ViolationPriority.LOW, "Illegal Sight: " + String.valueOf(n)));
            n6 = 0;
        }
